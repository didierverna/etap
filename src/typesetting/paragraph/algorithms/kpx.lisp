...
(define-kpx-caliber line-penalty -100 10 100)
...
(define-kpx-caliber adjacent-demerits 0 10000 20000 :bounded :min)
(define-kpx-caliber double-hyphen-demerits 0 10000 20000 :bounded :min)
(define-kpx-caliber final-hyphen-demerits 0 5000 20000 :bounded :min)
(define-kpx-caliber similar-demerits 0 5000 20000 :bounded :min)
(define-kpx-caliber pre-tolerance -1 100 10000 :infinity :max :bounded :min)
...
(define-kpx-caliber runt-threshold 0 15 50 :bounded t)
(define-kpx-caliber full-out-threshold 0 5 50 :bounded t)
...
(define-global-variables variant fitness line-penalty
  ...
  runt-threshold full-out-threshold)
...
(defun kpx-initialize-boundary (boundary)
  "Initialize BOUNDARY's (extended) fitness class, badness, and local demerits."
  (kp-initialize-boundary boundary)
  (setf (slot-value boundary 'extended-fitness-class)
	(kpx-sar-fitness-class (tsar boundary))))

;; This is used to update EOP boundaries to a single possible target width.
(defmethod reinitialize-instance :after ((boundary kpx-boundary) &key)
  "Reinitialize KPX BOUNDARY with a new TSAR."
  (kpx-initialize-boundary boundary))
    ...
    (cond
	 ...
	 (reinitialize-instance boundary
      ...
      ((and (= (min-width boundary) full-out) (< (max-width boundary) width))
       (unless (= (min-width boundary) (max-width boundary))
	 (reinitialize-instance boundary
      ...
      ((and (> (min-width boundary) full-out) (>= (max-width boundary) width))
       (unless (= (min-width boundary) (max-width boundary) width)
	 (reinitialize-instance boundary
...
(defmethod break-lineup
    (lineup width (algorithm (eql :kpx))
     &key ((:variant *variant*))
	  ...
	  ((:full-out-threshold *full-out-threshold*)))
  ...
  (calibrate-kpx full-out-threshold)
  (ecase *variant*
    (:graph
