...
(defun make-lineup
    (&key (context *context*)
	  (buffer (if context (buffer context) *text*))
...
- Explicit features take precedence over FEATURES."
  ...
  (%make-lineup buffer language font features disposition algorithm))
...
- Providing any option except for :context, :width, and :lineup will force
  ...
  (declare (ignore buffer language font kerning ligatures hyphenation
		   disposition algorithm))
  (when (or (null lineup)
	    bufferp languagep fontp
